...
open class Snapshot: NSObject {
    ...
    open class func snapshot(_ name: String, timeWaitingForIdle timeout: TimeInterval = 20) {
        ...
        #else
        let imageData: Data? = screenshot.pngRepresentation
        #endif

        guard let imageData else { return }

        var deviceName = ProcessInfo().environment["SIMULATOR_DEVICE_NAME"] ?? "macOS"
        do {
            ...
            NSLog("Problem writing screenshot: \(name) to \(screenshotsDir)/\(deviceName)-\(name).png")
            NSLog(error.localizedDescription)
        }
    }
